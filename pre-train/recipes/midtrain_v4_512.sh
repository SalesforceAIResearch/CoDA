...
# sample recipe to run midtrain on TPU v4-512
python torchprime/torch_xla_models/train.py \
    run_name=<run_name> \
    training_mode=pretrain \
    reshape_context=false \
    seg_attn=false \
    data=<midtrain_dataset> \
    model=coda-qwen-1b-tpu-v4 \
    ...
    max_steps=50000 \
    checkpoint_load_dir=gs://<bucket_name>/checkpoints/<pretrain_checkpoint_dir>/ \
    checkpoint_load_step=<pretrain_checkpoint_step> \
    resume_from_checkpoint=false \
    checkpoint_save_dir=gs://<bucket_name>/checkpoints/<midtrain_checkpoint_dir>/ \
    save_steps=1500 \
    logging_steps=1 \
    ici_mesh.fsdp=256 \
    ici_mesh.tensor=1 \
