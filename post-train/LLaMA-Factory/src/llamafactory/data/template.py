            ...
            if isinstance(elem, str):
                if len(elem) != 0:
                    try:
                        ...
                        # Filter out None values that might be introduced by TorchPrime tokenizers
                        encoded = [token_id for token_id in encoded if token_id is not None]
                        token_ids += encoded
                    except Exception as e:
                        # If encoding fails, skip this element
                        logger.warning(f"Failed to encode string '{elem}': {e}")
                        continue
            elif isinstance(elem, dict):
                token = elem.get("token")
                if token is None:
                    logger.warning(f"Token is None in element: {elem}")
                    continue
                try:
                ...
                if "bos_token" in elem:
                    if tokenizer.bos_token_id is not None:
                        ...
                        raise ValueError("BOS token requested but tokenizer.bos_token_id is None")
                elif "eos_token" in elem:
                    if tokenizer.eos_token_id is not None:
                        token_ids += [tokenizer.eos_token_id]
                    else:
                        raise ValueError("EOS token requested but tokenizer.eos_token_id is None")
            else:
